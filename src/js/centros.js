...
export function renderCC(){
  ...
  const fbarEl=document.querySelector('#centros .fbar');
...
export function abrirMCC(){
  _ecc=null;
  document.getElementById('mcc-t').textContent='🏢 Novo Centro';
  ['mcc-n','mcc-d'].forEach(id=>document.getElementById(id).value='');
  document.getElementById('mcc-s').value='ativo';
  window.populateSel();
  // Operacional: pré-seleciona e bloqueia o contrato (C.ct já vem filtrado pelo loadAll)
  const ctEl=document.getElementById('mcc-ct');
  if(SESSION?.perfil==='operacional'&&SESSION?.contrato_id){
    ctEl.value=SESSION.contrato_id;
    ctEl.disabled=true;
  } else {
    ctEl.disabled=false;
  }
