...
export function onMvFrota(){
  ...
  document.getElementById('mv-locador-box').style.display=frota==='locada'?'block':'none';
  if(frota!=='locada'){['mv-loc-nome','mv-loc-email','mv-loc-fone'].forEach(id=>document.getElementById(id).value='');}
}

export function abrirMV(){
  _ev=null;document.getElementById('mv-t').textContent='🚗 Novo Veículo';
  ['mv-placa','mv-modelo','mv-cor','mv-renavan','mv-resp','mv-obs','mv-med-nome','mv-med-local','mv-loc-nome','mv-loc-email','mv-loc-fone'].forEach(id=>document.getElementById(id).value='');
...
export function editV(id){
  ...
  document.getElementById('mv-frota').value=_ev.tipo_frota||'propria';
  ...
  document.getElementById('mv-locador-box').style.display=(_ev.tipo_frota==='locada')?'block':'none';
