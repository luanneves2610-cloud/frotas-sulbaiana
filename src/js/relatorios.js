...
export function previewRel(tipo){
  ...
  if(tipo==='analitico'){
    ...
    headers=['Placa','Modelo','Classificação','Contrato','Localidade','Abastec.','OS','Comb R$','Manut R$','Total R$'];
    rows=veics.map(v=>{
      const cm=manut.filter(m=>m.veiculo_id==v.id).reduce((s,m)=>s+Number(m.valor),0);
      const ca=abast.filter(a=>a.veiculo_id==v.id).reduce((s,a)=>s+Number(a.valor_total),0);
      const cls=(v.classificacao||'producao')==='producao'?'🚛 Produção':'🏭 Unidade Fixa';
      return[v.placa,v.modelo,cls,gCT(v.contrato_id).nome_contrato,gLoc(v.localidade_id).nome_localidade,abast.filter(a=>a.veiculo_id==v.id).length,manut.filter(m=>m.veiculo_id==v.id).length,cur(ca),cur(cm),cur(cm+ca)];
    }).sort((a,b)=>b[9]>a[9]?1:-1);
  } else if(tipo==='mensal'){
